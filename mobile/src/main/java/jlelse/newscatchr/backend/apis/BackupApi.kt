		...
		await { tryOrNull { key.downloadHaste() } }.let { json ->
			if (json != null) {
				...
				tryOrNull { Ason(json) }?.let { restoreAson ->
					await { tryOrNull { database.allFavorites = restoreAson.get("favorites", Array<Feed>::class.java).toMutableList() } }
					await { tryOrNull { database.allBookmarks = restoreAson.get("bookmarks", Array<Article>::class.java).toMutableList() } }
					await { tryOrNull { database.allReadUrls = restoreAson.get("readUrls", Array<String>::class.java).toMutableList() } }
				}
				progressDialog.dismiss()
